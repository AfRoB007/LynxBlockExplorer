...
const bitcoinApp = ()=>{
    ...
    app.get('*', hasAccess, function (req, res) {
        ...
        console.log('bitcoin middleware 5');
        let command = [];
        if (method == "sendmany") {
        ...
        console.log('bitcoin middleware 6',command);
        client.command(command, function (err, response) {
            if (err) {
                ...
                console.log('bitcoin middleware 7',response);
                let data = response;
